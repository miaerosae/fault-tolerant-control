...
class Env(BaseEnv):
    def __init__(self):
        ...
        self.controller = leso.Controller(self.plant.m,
                                          self.plant.g)
        self.B = get_B(self.plant.m, self.plant.g, self.plant.J)
        # TODO
        r = 
        K = 
        M1 = 
        alp1 = 
        self.ux = mm.mismatchESO(4, r, K, M1, alp1)
        self.uy = mm.mismatchESO(4, r, K, M1, alp1)
        self.uz = mm.mismatchESO(4, r, K, M1, alp1)
        self.upsi = mm.mismatchESO(2, r, K, M1, alp1)
    ...
    def set_dot(self, t):
        ...
        obs_ctrl[2] = self.ux.get_virtual(t, obs_ref[0, :, :])
        obs_ctrl[1] = self.uy.get_virtual(t, obs_ref[1, :, :])
        obs_ctrl[0] = self.uz.get_virtual(t, obs_ref[2, :, :])
        obs_ctrl[3] = self.upsi.get_virtual(obs_ref[3, 0:2, :])
        # TODO: can I use B0 instead of B, the paper says no, but if can,
        # I can use this for my paper
        obs_ctrl = np.linalg.inv(self.B).dot(obs_ctrl)
        ...
        self.ux.set_dot(t, y[0], obs_ref[0])
        self.uy.set_dot(t, y[1], obs_ref[1])
        self.uz.set_dot(t, y[2], obs_ref[2])
        self.upsi.set_dot(t, y[3], obs_ref[3])

        return dict(t=t, x=self.plant.observe_dict(), What=What,
                    rotors=rotors, rotors_cmd=rotors_cmd, W=W, ref=ref,
                    obs_u=obs_ctrl, virtual_u=forces)
