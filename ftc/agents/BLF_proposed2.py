...
class outerLoop(BaseEnv):
    ...
    def deriv(self, e, eta, y, ref, t):
        ...
        edot = np.zeros((3, 1))
        edot[0, :] = eta[0] + alp[0]/l * func_g(l**2*(e_real - e[0]), theta[0], delta)
        edot[1, :] = (eta[1] + q
                      + alp[1] * func_g(l**2*(sat(eta[0], R[0]) - e[1]), theta[1], delta))
        edot[2, :] = alp[2] * l * func_g(l**2*(sat(eta[1], R[1]) - e[2]), theta[2], delta)

        etadot = np.zeros((2, 1))
        etadot[0, :] = (sat(eta[1], R[1]) + q
                        + bet[0] * func_g(l**2*(e_real - e[0]), theta[0], delta))
        etadot[1, :] = bet[1] * l * func_g(l**2*(sat(eta[0], R[0]) - e[1]), theta[1], delta)
...
class innerLoop(BaseEnv):
    ...
    def deriv(self, x, eta, lamb, t, y, ref):
        ...
        xdot = np.zeros((3, 1))
        xdot[0, :] = eta[0] + alp[0]/l * func_g(l**2*(y - x[0]), theta[0], delta)
        xdot[1, :] = (eta[1] + nu_sat
                      + alp[1] * func_g(l**2*(sat(eta[0], R[0]) - x[1]), theta[1], delta))
        xdot[2, :] = alp[2] * l * func_g(l**2*(sat(eta[1], R[1]) - x[2]), theta[2], delta)

        etadot = np.zeros((2, 1))
        etadot[0, :] = (sat(eta[1], R[1]) + nu_sat
                        + bet[0] * func_g(l**2*(y - x[0]), theta[0], delta))
        etadot[1, :] = bet[1] * l * func_g(l**2*(sat(eta[0], R[0]) - x[1]), theta[1], delta)
