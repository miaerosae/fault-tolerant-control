...
class lowPowerESO(BaseEnv):
            ...
            for i in range(nu-2):
                if i == 0:
                    xidot[j, i, :, :] = A.dot(xi[j, i, :, :]) + N.dot(xi[j, i+1, :, :]) \
                        + D.dot(K[j, :, :])*(y[j]-C.dot(xi[j, i, :, :]))
                else:
                    xidot[j, i, :, :] = A.dot(xi[j, i, :, :]) + N.dot(xi[j, i+1, :, :]) \
                        + D.dot(K[j, :, :])*(B.T.dot(xi[j, i-1, :, :])-C.dot(xi[j, i, :, :]))
    ...
    def get_obs(self):
        sn = np.size(self.n)
        observe = np.reshape(self.xi.state[:, 0, 0, :], [sn, 1])
        return observe
...
if __name__ == "__main__":
    print(satv(np.array([100, 1, 1]), np.vstack([150, -10, 10])))
