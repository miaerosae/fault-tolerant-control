...
class DOB(BaseEnv):
    ...
    def deriv(self, xi, sig, y, L):
        ...
        xidot[n-2, :, :] = A.dot(xi[n-2, :, :]) + N.dot(sig) \
            ...
            + D.dot(K[n-1, :, :].dot(B.T.dot(xi[n-3, :, :])-C.dot(xi[n-2, :, :])))
        ...
        return xidot, sigdot, ctrl

    def set_dot(self, t, y, L):
        states = self.observe_list()
        dots = self.deriv(*states, y, L)
        self.xi.dot, self.sig.dot, _ = dots

    def control(self, y, L):
        states = self.observe_list()
        dots = self.deriv(*states, y, L)
        *_, ctrl = dots
        return ctrl
...
if __name__ == "__main__":
    system = DOB(4, 3, np.array([[1, 1], [2, 2], [3, 3], [4, 4]]), 3, np.array([[1, 2, 3, 4]]))
