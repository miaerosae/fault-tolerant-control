...
class outerLoop(BaseEnv):
    ...
    def deriv(self, e, eta, integ_e, y, ref, t):
        ...
        edot[1, :] = (eta[1] + q
                      + alp[1] * func_g(l*(eta[0] - e[1]), theta[1], delta))
        edot[2, :] = alp[2] * func_g(l*(eta[1] - e[2]), theta[2], delta)

        etadot = np.zeros((2, 1))
        etadot[0, :] = (eta[1] + q
                        + bet[0] * l*func_g(l*(e_real - e[0]), theta[0], delta))
        etadot[1, :] = bet[1] * l*func_g(l*(eta[0] - e[1]), theta[1], delta)
...
class innerLoop(BaseEnv):
    ...
    def deriv(self, x, eta, lamb, integ_e, t, y, ref):
        ...
        xdot[1, :] = (eta[1] + nu_sat
                      + alp[1] * func_g(l*(eta[0] - x[1]), theta[1], delta))
        xdot[2, :] = alp[2] * func_g(l*(eta[1] - x[2]), theta[2], delta)

        etadot = np.zeros((2, 1))
        etadot[0, :] = (eta[1] + nu_sat
                        + bet[0] * l*func_g(l*(y - x[0]), theta[0], delta))
        etadot[1, :] = bet[1] * l*func_g(l*(eta[0] - x[1]), theta[1], delta)
