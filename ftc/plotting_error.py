...
def exp_plot(loggerpath):
    ...
    ax = plt.subplot(411)
    for i, _label in enumerate([r"$err_{1}$", r"$err_{2}$", r"$err_{3}$", r"$err_{4}$"]):
        if i != 0:
            plt.subplot(411+i, sharex=ax)
        plt.plot(data["t"], data["err"][:, i], "r", label=_label)
        plt.plot(data["t"], data["err_obs"][:, i], "k--", label=_label)
        plt.legend(loc="upper right")
    plt.gcf().supxlabel("Time, sec")
    plt.gcf().supylabel("Error Estimation")
    plt.tight_layout()
